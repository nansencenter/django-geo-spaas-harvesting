...
The `providers` subcommand allows to list, create and delete providers.

###### -l, --list
Print the available providers.
```shell
python -m geospaas_harvesting.cli -c ./config.yml providers list
```

###### -d, --delete <provider_name1> <provider_name2> ...
Delete one or more providers.

###### -u, --update <providers_path>
Create providers from a YAML file containing provider configurations.
...
- **providers_path**: path to a file containing the default providers' configuration.

#### Providers configuration file (default: [default_providers.yml](./geospaas_harvesting/default_providers.yml))

The properties which are common to every harvester are:

- **providers** (mandatory): Dictionary mapping providers names to configurations.

A provider's configuration is a dictionary containing the following keys:
- **crawler**: crawler's configuration. Contains at least a **name**.
- **normalizer**: normalizer's configuration. Contains at least a **name**.
- **ingester**: ingester's options.
...
- **searches**: a list of dictionaries, each containing search parameters suited to a provider.
  ...
  The search parameters have the same structure as providers' configurations and can override a provider's default configuration.
...
common: # these are common to all searches
  time_range: ['2022-07-13', '2022-07-14']
...
- provider_name: 'earthdata_cmr'
  crawler:
    ...
    time_range: ['2018-12-01T00:00:00Z', '2018-12-04T12:00:00Z']
```
